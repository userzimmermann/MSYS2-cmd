...
if "%~1" == "/?" (
    echo Runs MSYS2 'makepkg-mingw' script via bash.exe
    echo.
    echo MAKEPKG-MINGW [args ...]
    echo.
    echo First starts a non-interactive bash.exe with --login flag,
    echo which then switches to current working directory
    echo and runs 'makepkg-mingw' with the given arguments.
    exit /b 0
)

"%~dp0bash.bat" -c "makepkg-mingw %*"
